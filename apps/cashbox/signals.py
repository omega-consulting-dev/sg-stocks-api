...
def create_cashbox_for_store(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if not Cashbox.objects.filter(store=instance, is_active=True).exists():
            # Toujours créer une NOUVELLE caisse avec un code unique (inclut timestamp)
            # Format: CAISSE-STR-001-1708345678 (code unique par création)
            timestamp = int(timezone.now().timestamp())
            cashbox_code = f"CAISSE-{instance.code}-{timestamp}"
