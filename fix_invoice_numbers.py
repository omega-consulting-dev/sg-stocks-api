...
def fix_incorrect_invoice_numbers(schema_name):
    """
    Corrige les numéros de facture dont l'année ne correspond pas
    à l'année de invoice_date - réordonne toutes les factures par année
    """
    with schema_context(schema_name):
        print(f"\n{'='*60}")
        print(f"Correction des numéros de facture pour: {schema_name}")
        print(f"{'='*60}\n")
        ...
        for year in sorted(invoices_by_year.keys()):
            ...
            # Première passe: renommer en temporaire pour éviter les conflits
            temp_numbers = {}
        ...
        print(f"\n{'='*60}")
        print(f"Résumé:")
        print(f"  - Factures mises à jour: {updated}")
        print(f"  - Erreurs: {errors}")
        print(f"{'='*60}\n")

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python fix_invoice_numbers.py <schema_name>")
        print("Exemple: python fix_invoice_numbers.py agribio")
        sys.exit(1)
    ...
    try:
        ...
        fix_incorrect_invoice_numbers(schema_name)
